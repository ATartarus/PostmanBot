...
function createInlineKeyboard(buttons, cols, state, labels) {
    ...
    for (let i = 0; i < buttons; i++) {
        ...
        const text = labels ? labels[i] : (i + 1).toString();
        row.push({ text: text !== null && text !== void 0 ? text : (i + 1).toString(), callback_data: `${state}:${i}` });
    }
    if (row.length > 0) {
        keyboard.push(row);
    }
